Shader "Custom/ForwardsRadial_Vert"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			vf vert(const appdata v)
			{
				...
				// Perform Brown's simplified forward radial transform for the vertex position of this vertex in clip space
				// (-1.0 to 1.0) for xy, (0.0) for z, (1.0) for w
				o.vertex = float4(forward_point(vertex.xy), vertex.zw);
				o.uv = v.uv;
				return o;
			}
